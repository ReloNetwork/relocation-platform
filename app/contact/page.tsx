      ...
      <main className="bg-[#f4f0e7] text-[#14291f]">
        <section className="border-b border-[#d8d0c2] px-5 pb-16 pt-28 sm:pt-36">
          <div className="mx-auto max-w-5xl">
            <p className="text-xs font-semibold tracking-[0.22em] text-[#725449]">
              CONTACT
            </p>
            <h1 className="mt-5 max-w-4xl font-serif text-5xl leading-[0.98] sm:text-7xl">
              LET&apos;S TALK ABOUT YOUR LONDON PLANS.
            </h1>
            <p className="mt-7 max-w-2xl text-xl leading-8 text-[#4f5b53]">
              Choose the route that fits. You will reach the right form without
        ...
        <section className="mx-auto grid max-w-5xl gap-px bg-[#d8d0c2] px-5 py-16 sm:grid-cols-3 sm:px-0">
          {routes.map((route) => (
            <article
              className="flex min-h-80 flex-col bg-[#f4f0e7] p-7"
              key={route.number}
            >
              <span className="text-xs font-semibold tracking-[0.18em] text-[#725449]">
                {route.number}
              </span>
              <h2 className="mt-8 font-serif text-3xl">{route.title}</h2>
              <p className="mt-5 flex-1 leading-7 text-[#536057]">
                {route.text}
              </p>
              <Link
                className="mt-8 text-sm font-semibold uppercase tracking-[0.12em] underline underline-offset-4"
                href={route.href}
              >
                {route.action}
              </Link>
        ...
        <section className="border-t border-[#d8d0c2] px-5 py-12 text-center text-sm leading-6 text-[#606b63]">
          The Relo Network is not an emergency service. If someone is in
          immediate danger, contact the appropriate emergency service.
        </section>
      </main>
    </Layout>
  );
