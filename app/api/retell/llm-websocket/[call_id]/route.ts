...
// Path parameter: /api/retell/llm-websocket/[call_id]
// This follows Retell's WebSocket endpoint specification
...
export async function POST(
  ...
  try {
    ...
    console.log(`[${call_id}] Received event: ${event.interaction_type}`)
    
    // Handle different event types according to Retell protocol
    switch (event.interaction_type) {
      ...
      case 'reminder_required':
        ...
        console.log(`[${call_id}] Generated response for ${event.interaction_type}`)
      ...
      case 'update_only':
        ...
        console.log(`[${call_id}] Transcript update received`)
        return NextResponse.json({ status: 'acknowledged' })
        
      default:
        console.warn(`[${call_id}] Unknown interaction type: ${event.interaction_type}`)
        return NextResponse.json({ status: 'unknown_event' })
    }

  } catch (error) {
    console.error(`[${params.call_id}] LLM websocket error:`, error)
